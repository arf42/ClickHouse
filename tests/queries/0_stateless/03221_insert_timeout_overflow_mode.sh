...
${CLICKHOUSE_CLIENT} --query "select number + sleep(0.1) as number from system.numbers limit 10 settings max_block_size = 1 format Native" | ${CLICKHOUSE_CLIENT} --max_threads=2 --max_execution_time 0.3 --timeout_overflow_mode break --query "insert into dst format Native"
